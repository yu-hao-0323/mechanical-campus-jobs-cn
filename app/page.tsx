...
export default function Home() {
  ...
  const resendVerification = async () => {
    if (!supabase || !authEmail) { setResumeMessage('请先填写注册邮箱。'); return; }
    setResumeBusy(true);
    const { error } = await supabase.auth.resend({ type: 'signup', email: authEmail, options: { emailRedirectTo: `${window.location.origin}${window.location.pathname}` } });
    setResumeBusy(false);
    setResumeMessage(error ? `发送失败：${error.message}` : '新的验证邮件已发送，请使用这封邮件中的链接。');
  };
